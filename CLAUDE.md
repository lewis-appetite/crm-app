...
- Sorted (client-side, `OutreachApp.tsx`, explicit tiers — not relying on array stability): 1) region/time-of-day preference (same `regionSortRank` as New), 2) Focus-shortlisted company, 3) underlying cadence order (replied before no-reply-yet, then longest-since-last-contacted — `compareFollowUpCadence` in `sheets.ts`, shared with `getFollowUpQueue`'s own base sort), 4) active A/B test as the final tiebreaker.

**New contacts queue** — contacts where:
- `message` is empty AND `lastContacted` is empty AND not dead

**Replied queue** (`getRepliedQueue`) — every contact with `reply` in Interested/Yes and no `Call booked`, sorted oldest-last-contacted first. Deliberately not cadence-gated like Follow-ups (`FOLLOW_UP_INTERVAL_DAYS` doesn't apply) — this is the "don't lose track of a live conversation" view, surfaced as the **Replied** tab in the "⋯" menu. A contact drops off the moment a meeting is booked or the reply is changed away from Interested/Yes. Its "Log follow-up" button calls the same `handleFocusDone(c)` as Focus's "Done" button — one click writes a `One-off` template into the next open Follow Up Message slot and bumps `Follow ups`/`Last Contacted`, no note or comment involved.
...
- **Region-based time-of-day sort**: `getRegionMode`/`normalizeRegion`/`regionSortRank` in `sheets.ts`. Matches `United Kingdom`/`UK`/`GB`/`Great Britain` and `United States`/`US`/`USA`. UK and US both outrank every other country (and blank) regardless of which one is currently preferred — rank 0 = matches the current time window, rank 1 = the other of UK/US, rank 2 = anything else (a genuine third country like Ireland/Germany, or blank). 06:00–21:59 local time favors UK contacts in New/Follow-ups ordering, 22:00–05:59 favors US; header shows a live "🇬🇧 UK hours"/"🇺🇸 US hours" chip. Follow-ups queue is re-sorted client-side (stable sort layered on the server's cadence-priority order); New tab sorts region rank between the cake-image check and the user's chosen sort. The chip is also a manual override toggle (`regionOverride` state in `OutreachApp.tsx`, click to cycle auto → UK → US → auto) — pins `regionMode` to a region regardless of wall-clock time, styled distinctly (`regionChipOverride`, "UK/US priority" label) when active; resets to automatic time-of-day banding on every fresh page load, it isn't persisted.
...
- **Focus-company prioritization** (superseded the original overlap/exclusion rule): Focus-shortlisted companies' contacts now show in Follow-ups and New *as well as* Focus, not instead of — `OutreachApp.tsx` computes `focusedCompanyKeys` client-side from `data.campaigns` and pins them to the top of both queues' client-side stable sort (above cake-image match in New; in Follow-ups it's the second tier, below region/time-of-day but above the underlying cadence order and A/B-test pinning — see Key Logic above for the full Follow-up sort order). `getFollowUpQueue` no longer takes an exclusion set. `TierContact.followUpDue` (computed via the shared `isFollowUpDue` predicate) still badges "Follow-up also due" within Focus itself.
