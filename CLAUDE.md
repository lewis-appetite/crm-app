...
- **Focus-company prioritization** (superseded the original overlap/exclusion rule): Focus-shortlisted companies' contacts now show in Follow-ups and New *as well as* Focus, not instead of — `OutreachApp.tsx` computes `focusedCompanyKeys` client-side from `data.campaigns` and pins them to the very top of both queues' client-side stable sort (above cake-image match in New; above A/B-test pinning in Follow-ups). `getFollowUpQueue` no longer takes an exclusion set. `TierContact.followUpDue` (computed via the shared `isFollowUpDue` predicate) still badges "Follow-up also due" within Focus itself.
